...
class ChatGLMPreTrainedModel(PreTrainedModel):
    ...
    config_class = ChatGLMConfig # 模型配置类
...
class ChatGLMModel(ChatGLMPreTrainedModel):
    """
    该模型可以充当编码器（仅具有自注意力）以及解码器，在这种情况下，在自注意力层之间添加了一个交叉注意力层，
    遵循 Ashish Vaswani、Noam Shazeer、Niki Parmar、Jakob Uszkoreit、Llion Jones、Aidan N. Gomez、Lukasz Kaiser 和 Illia Polosukhin
    在 [Attention is all you need](https://arxiv.org/abs/1706.03762) 中描述的架构。
    要充当解码器，需要将配置的 `is_decoder` 参数设置为 `True` 来初始化模型。
    要在 Seq2Seq 模型中使用，需要将 `is_decoder` 参数和 `add_cross_attention` 设置为 `True` 来初始化模型；
    然后预期 `encoder_hidden_states` 作为前向传递的输入。
    """
